...
public class IsProperSubset extends SetOperator {

    public IsProperSubset(Expression ex1, Expression ex2) {
        super(ex1, ex2);
    }

    @Override
    protected RQLSymbol setOperation(RQLSet s1, RQLSet s2) throws IllegalExpressionException {
        return new LongRQLVariable(s2.isSubset(s1) && s2.size() < s1.size() ? 1 : 0);
    }
