...
public class DistributedQuerySetStructTest {
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < LEN; i++) {
            RQLStruct s = log.createStruct().addValue("var1", i*i).addValue("var2", i);
            log.addToSet("struct", s).commit();
        }
    ...
    public void testSimpleDistributedQuery() throws Exception {
        StringReader q1 = new StringReader("SELECT struct FROM testlog WHEN EXISTS s IN Flatten(struct) : (s.var1 > 16);");
