        ...
        if (params.getList().size() == 1){
            ...
            for (int i = 0; i < p1Vals.length; i++) {
                ...
                switch (p1Vals[i].getType()) {
                    case INT:
                        expressionValue.setValFloat(Math.tan(p1Vals[i].getIntVal()));
                        break;
                    case DOUBLE:
                        expressionValue.setValFloat(Math.tan(p1Vals[i].getDoubleVal()));
                        break;
                    default:
                        RQLRunTimeWarning w = new RQLRunTimeWarning(
                                RQLRunTimeWarning.WarningType.INCOMPATIBLE_TYPES,
                                this.getClass().getName() + this.hashCode(),
                                "Function Tan is undefined for " + p1Vals[i].getType()
                        );
